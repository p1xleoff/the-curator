  ...
  const openCamera = async () => {
    ...
    if (!result.canceled) {
      const newMedia: MediaItem[] = result.assets.map((asset) => ({
        ...
        duration: asset.type === 'video' ? Math.round((asset.duration || 0) / 1000) : undefined,
      }));
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <ThemedInput multiline={true} name='review' placeholder='Share details about the product...' />
      </View>
      <View style={styles.section}>
        <ThemedText type='subtitle'>Add Product Pruchase Details</ThemedText>
        <ThemedText type='default' >Which store did you purchase the product from</ThemedText>
        <ThemedInput multiline={false} name='store' />
        <ThemedText>How much did you pay for the product</ThemedText>
        <ThemedInput multiline={false} name='price' placeholder='Price in Rupees' />
